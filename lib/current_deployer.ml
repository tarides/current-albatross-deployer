...
module Info = struct
  type t = { ip : Ipaddr.V4.t }

  let ip { ip } = ip
end

module Args = struct
  type t = { get : (Ipaddr.V4.t -> string list) Current.t }
...
module E = struct
  ...
  module Info = struct
    ...
    let pp f { config; ip; _ } =
      Fmt.pf f "%s @%a" config.service Ipaddr.V4.pp ip
  end
  ...
  module IpOp = struct
    ...
    let build No_context job key =
      ...
      let** ip =
        Lwt.finalize
          (fun () ->
            Client.IpManager.request ~socket key |> Lwt.map remap_errors)
  ...
  let deploy_albatross config =
    ...
    let monitor =
      ...
      |> let> config = config and> () = deploy in
         ...
         let ip = config.ip in
         let read () =
           Lwt_result.return { Info.status = `Running; ip; config }
         in
         let watch _refresh = Lwt.return (fun () -> Lwt.return ()) in
         let pp f =
           Fmt.pf f "Monitor deployment %s on %a" name Ipaddr.V4.pp ip
         in
    ...
    let build No_context job roots =
      ...
      let perform ~socket =
        ...
        let* () =
          Lwt_list.iter_s
            (fun (deployment : Current_deployer_api.Types.DeploymentInfo.t) ->
              ...
              Client.Deployments.remove ~socket deployment.name
              |> Lwt.map ignore)
        ...
        let removed_ips_tags =
          List.map
            (fun ip -> ip.Current_deployer_api.Types.Ip.tag)
            ips_to_remove
          |> StringSet.of_list
        in
        (* Step 2: albatross: remove unikernels starting by a deployment's
           service name *)
